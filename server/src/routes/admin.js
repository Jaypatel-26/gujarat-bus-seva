...
async function bookingsQuery(req) {
  ...
  if (q) {
    where.OR = [
      ...
      { passengers: { some: { mobile: { contains: q } } } }, // ... ya uske mobile se
    ];
  }
...
r.get("/bookings.csv", wrap(async (req, res) => {
  ...
  const lines = [["PNR", "Passengers (seat time)", "Booked By", "Booker Mobile", "Booker Email", "Route", "Journey Date", "Bus", "Seats", "Fare", "Status", "Payment", "Booked At"].join(",")];
  for (const b of rows) {
    lines.push([
      b.pnr,
      esc(b.passengers.map((p) => `${p.name} (${p.age}/${p.gender}${p.mobile ? `, ${p.mobile}` : ""}${p.email ? `, ${p.email}` : ""})`).join("; ")),
      esc(b.user?.name), b.user?.mobile, b.user?.email || "",
